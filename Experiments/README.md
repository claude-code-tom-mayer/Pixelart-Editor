...
Measurement code only. **Nothing here is wired into `Servers/`** — the shipped
...
> Only the GDScript side is kept in `Src/`. The C++, Rust and C# ports were removed
> and will be rebuilt; the native bench scripts need them (`SearchKernel`,
> `SearchKernelRs`, and the C# kernel uid in `BenchCS.gd`) before they run again.
> The numbers below were measured with the removed ports.
...
- **godot-rust**: the released gdext 0.4.5 cannot generate bindings for Godot
  ...
  (0.5.5) works. The removed `gdext_Cargo.toml` pinned it, with the
...
- GDScript: put `Src/KernelGD.gd`, `Src/KernelGDOpt.gd`, `Src/HitGD.gd` and `Src/NoopGD.gd`
  in a project; every `Src/Bench*.gd` runs headless once its native kernel is loaded.
- C++: build `godot-cpp` against the engine's own `--dump-extension-api` output and
  register a `SearchKernel` class with `build`, `run`, `build_hits`, `run_hits` and `noop`.
- Rust GDExtension: the same interface as `SearchKernelRs`, gdext git master.
- C#: a Godot .NET project with the kernel script; set its uid in `BenchCS.gd`.
...
the removed `hit_core.h` and `Src/HitGD.gd` resolve one hit per entity per frame from a
