extends A_ExpEntityServer
class_name ExpTargetingServer
...
func register(p_id: int, p_targetingData: R_TargetingData) -> void:
	...
	if (l_targetingData == null):
		push_error("ExpTargetingServer: entity %d was registered without targeting data, using the defaults." % p_id)
...
## Overrides A_ExpEntityServer. [br]
...
func _can_still_flee(p_id: int) -> bool:
	var l_baseX: float = TEAM_BASE_X[_chunking._entityTeam[p_id]]
	return absf(_chunking._entityPosition[p_id].x - l_baseX) > BASE_REACHED_EPSILON
...
func _scan_chunk(p_chunkId: int) -> void:
	...
	for l_team: int in TEAM_COUNT:
		if (l_team == _searchTeam):
			continue
		
		var l_candidateId: int = _chunking._centerHead[l_team * CHUNK_COUNT + p_chunkId]
