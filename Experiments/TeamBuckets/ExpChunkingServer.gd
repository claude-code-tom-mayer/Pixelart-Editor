...
func _init() -> void:
	...
	_centerHead.resize(TEAM_COUNT * CHUNK_COUNT)
...
func register_entity(p_position: Vector2, p_radius: float, p_team: int, p_groups: int) -> int:
	assert(p_team >= 0 and p_team < TEAM_COUNT, "ExpChunkingServer: register_entity() got an unknown team.")
...
func _apply_center_chunk(p_id: int, p_position: Vector2) -> void:
	...
	var l_headIndex: int = _entityTeam[p_id] * CHUNK_COUNT + l_chunkId
...
func _remove_center_from_chunk(p_id: int) -> void:
	...
	if (l_prev == NO_ENTITY):
		_centerHead[_entityTeam[p_id] * CHUNK_COUNT + l_chunkId] = l_next
	else:
